from utils.nn import PtConv
from utils.nn.utils import apply_bn
import torch
...
class SegBig(nn.Module):
    ...
    def forward(self, x, input_pts, return_features=False):
        ...
        :param x: tensor(B, N, input_features)
        :param input_pts: tensor(B, N, 3)
        :param return_features:
        :return:
        xout: tensor(B, N, out_channels)
        x0d[optional]: tensor(B, N, 2pl)
        """

        x0, _ = self.cv0(x, input_pts, 16)  # (B, N, pl)
        x0 = self.relu(apply_bn(x0, self.bn0))  # (B, N, pl)

        x1, pts1 = self.cv1(x0, input_pts, 16, 2048)  # (B, 2048, pl), (B, 2048, 3)
        x1 = self.relu(apply_bn(x1, self.bn1))

        x2, pts2 = self.cv2(x1, pts1, 16, 1024)  # (B, 1024, pl), (B, 1024, 3)
        x2 = self.relu(apply_bn(x2, self.bn2))

        x3, pts3 = self.cv3(x2, pts2, 16, 256)  # (B, 256, pl)
        x3 = self.relu(apply_bn(x3, self.bn3))

        x4, pts4 = self.cv4(x3, pts3, 8, 64)  # (B, 64, 2pl)
        x4 = self.relu(apply_bn(x4, self.bn4))

        x5, pts5 = self.cv5(x4, pts4, 8, 16)  # (B, 16, 2pl)
        x5 = self.relu(apply_bn(x5, self.bn5))

        x6, pts6 = self.cv6(x5, pts5, 4, 8)  # (B, 8, 2pl)
        x6 = self.relu(apply_bn(x6, self.bn6))

        x5d, _ = self.cv5d(x6, pts6, 4, pts5)  # (B, 16, 2pl)
        x5d = self.relu(apply_bn(x5d, self.bn5d))
        x5d = torch.cat([x5d, x5], dim=2)  # (B, 16, 4pl)

        x4d, _ = self.cv4d(x5d, pts5, 4, pts4)  # (B, 64, 2pl)
        x4d = self.relu(apply_bn(x4d, self.bn4d))
        x4d = torch.cat([x4d, x4], dim=2)

        x3d, _ = self.cv3d(x4d, pts4, 4, pts3)  # (B, 256, pl)
        x3d = self.relu(apply_bn(x3d, self.bn3d))
        x3d = torch.cat([x3d, x3], dim=2)

        x2d, _ = self.cv2d(x3d, pts3, 8, pts2)  # (B, 1024, pl)
        x2d = self.relu(apply_bn(x2d, self.bn2d))
        x2d = torch.cat([x2d, x2], dim=2)

        x1d, _ = self.cv1d(x2d, pts2, 8, pts1)  # (B, 2048, pl)
        x1d = self.relu(apply_bn(x1d, self.bn1d))
        x1d = torch.cat([x1d, x1], dim=2)

        x0d, _ = self.cv0d(x1d, pts1, 8, input_pts)  # (B, N, pl)
        x0d = self.relu(apply_bn(x0d, self.bn0d))

        x0d = torch.cat([x0d, x0], dim=2)  # (B, N, 2pl)
