...
class PBC_pure(nn.Module):
    def __init__(self, num_classes, with_rgb=False, addition_channel=0):
        super(PBC_pure, self).__init__()
        self.with_rgb = with_rgb
        self.feature_channel = addition_channel
        if with_rgb:
            self.feature_channel += 3
        print('feature_channel in PBC_pure is %d' % self.feature_channel)

        # Encoder
        # we discard the default setting radius [0.5,1.0,2.0,4.0] as we don't normalize point cloud before network
        self.sa1 = PNSADenseNet(1024, 0.5, 32, [self.feature_channel + 3, 64, 64], False, False)
        self.sa2 = PNSADenseNet(256, 1.0, 32, [64 + 3, 128, 128], False, False)
        self.sa3 = PNSADenseNet(64, 2.0, 32, [128 + 3, 256, 256], False, False)
        self.sa4 = PNSADenseNet(16, 4.0, 32, [256 + 3, 512, 512], False, False)
