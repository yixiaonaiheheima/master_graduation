...
from utils.atlas_utils import AtlasDec
from utils.capsule_utils import DecodeFromVec
from utils.PBC_util import PNSADenseNet, PNFPDenseNet


class PBC_folding(nn.Module):
    def __init__(self, num_classes, with_rgb=False, addition_channel=0):
        super(PBC_folding, self).__init__()
        self.with_rgb = with_rgb
        self.feature_channel = addition_channel
        if with_rgb:
            self.feature_channel += 3
        print("feature_channel in PBC_folding is %d" % self.feature_channel)
        # Encoder
        ...
        self.sa1 = PNSADenseNet(1024, 0.5, 32, [32 + 3, 64, 64], False, False)
        self.sa2 = PNSADenseNet(256, 1.0, 32, [64 + 3, 128, 128], False, False)
        self.sa3 = PNSADenseNet(64, 2.0, 16, [128 + 3, 256, 256], False, False)
        self.sa4 = PNSADenseNet(16, 4.0, 16, [256 + 3, 512, 512], False, False)
    ...
    def forward(self, pointcloud1, pointcloud2):
        ...
        l0_points1 = F.relu(self.bn0(self.conv0(pointcloud1.permute(0, 2, 1))))  # (B, 32, N)
        l0_points2 = F.relu(self.bn0(self.conv0(pointcloud2.permute(0, 2, 1))))  # (B, 32, N)
        l0_points1 = l0_points1.permute(0, 2, 1)  # (B, N, 32)
...
class PBC_atlas(nn.Module):
    def __init__(self, num_classes, with_rgb=False, addition_channel=0):
        super(PBC_atlas, self).__init__()
        self.with_rgb = with_rgb
        self.feature_channel = addition_channel
        if with_rgb:
            self.feature_channel += 3
        print("feature_channel in PBC_atlas is %d" % self.feature_channel)
        # Encoder
        self.conv0 = nn.Conv1d(self.feature_channel + 3, 32, 1)
        self.bn0 = nn.BatchNorm1d(32)
        # we discard the default setting radius [0.5,1.0,2.0,4.0] as we don't normalize point cloud before network
    ...
    def forward(self, pointcloud1, pointcloud2):
        ...
        # npm3d decoder (folding)
        global_feature, _ = torch.max(l4_points2, dim=-1)  # (b, 512)
        npm_pc_reconstructed = self.dec(global_feature)  # (b, 3, 90^2)
        npm_pc_reconstructed = npm_pc_reconstructed.permute(0, 2, 1)  # (b, 90^2, 3)

        return semantic3d_prob, npm_pc_reconstructed


class PBC_caps(nn.Module):
    def __init__(self, num_classes, with_rgb=False, addition_channel=0):
        super(PBC_caps, self).__init__()
        self.with_rgb = with_rgb
        self.feature_channel = addition_channel
        if with_rgb:
            self.feature_channel += 3
        print("feature_channel in PBC_caps is %d" % self.feature_channel)
        # Encoder
        self.conv0 = nn.Conv1d(self.feature_channel + 3, 64, 1)
        self.bn0 = nn.BatchNorm1d(64)
        # we discard the default setting radius [0.5,1.0,2.0,4.0] as we don't normalize point cloud before network
        self.sa1 = PNSADenseNet(1024, 0.5, 32, [64 + 3, 64, 64], False, False)
        self.sa2 = PNSADenseNet(256, 1.0, 32, [64 + 3, 128, 128], False, False)
        self.sa3 = PNSADenseNet(64, 2.0, 16, [128 + 3, 256, 256], False, False)
        self.sa4 = PNSADenseNet(16, 4.0, 16, [256 + 3, 512, 512], False, False)
    ...
    def forward(self, pointcloud1, pointcloud2):
        ...
        l0_points1 = F.relu(self.bn0(self.conv0(pointcloud1.permute(0, 2, 1))))
        l0_points2 = F.relu(self.bn0(self.conv0(pointcloud2.permute(0, 2, 1))))
        l0_points1 = l0_points1.permute(0, 2, 1)  # (B, N, 64)
        # semantic3d encoder (share weighted)
        l1_xyz1, l1_points1 = self.sa1(l0_xyz1, l0_points1)  # (b, 1024, 3), (b, 1024, 64)
        l2_xyz1, l2_points1 = self.sa2(l1_xyz1, l1_points1)  # (b, 256, 3), (b, 256, 128)
        l3_xyz1, l3_points1 = self.sa3(l2_xyz1, l2_points1)  # (b, 64, 3), (b, 64, 256)
        ...
        # npm3d encoder (share weighted)
        l2_points2 = F.relu(self.bn2(self.conv2(l0_points2)))
        ...
        # npm3d decoder (folding)
        # global_feature, _ = torch.max(l4_points2, dim=-1)  # (b, 512)
        npm_pc_reconstructed = self.dec(l2_points2)  # (b, 3, 90^2)
        npm_pc_reconstructed = npm_pc_reconstructed.permute(0, 2, 1)  # (b, 90^2, 3)
