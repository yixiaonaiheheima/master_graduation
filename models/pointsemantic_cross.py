...
class PointSemantic_folding(nn.Module):
    def __init__(self, num_classes, with_rgb=False, addition_channel=0):
        super(PointSemantic_folding, self).__init__()
        self.with_rgb = with_rgb
        self.feature_channel = addition_channel
        if with_rgb:
            self.feature_channel += 3
        print("feature_channel in PointSemantic_folding is %d" % self.feature_channel)
        # Encoder
...
class PointSemantic_atlas(nn.Module):
    def __init__(self, num_classes, with_rgb=False, addition_channel=0):
        super(PointSemantic_atlas, self).__init__()
        self.with_rgb = with_rgb
        self.feature_channel = addition_channel
        if with_rgb:
            self.feature_channel += 3
        print("feature_channel in PointSemantic_atlas is %d" % self.feature_channel)
        # Encoder
        self.conv0 = nn.Conv1d(self.feature_channel + 3, 32, 1)
        self.bn0 = nn.BatchNorm1d(32)
        # we discard the default setting radius [0.5,1.0,2.0,4.0] as we don't normalize point cloud before network
...
class PointSemantic_caps(nn.Module):
    def __init__(self, num_classes, with_rgb=False, addition_channel=0):
        super(PointSemantic_caps, self).__init__()
        self.with_rgb = with_rgb
        self.feature_channel = addition_channel
        if with_rgb:
            self.feature_channel += 3
        print("feature_channel in PointSemantic_caps is %d" % self.feature_channel)
        # Encoder
        self.conv0 = nn.Conv1d(self.feature_channel + 3, 64, 1)
        self.bn0 = nn.BatchNorm1d(64)
        # we discard the default setting radius [0.5,1.0,2.0,4.0] as we don't normalize point cloud before network
        self.sa1 = PNSADenseNet(1024, 0.5, 32, [2 * (64 + 3), 64, 64], False, False)
        self.sa2 = PNSADenseNet(256, 1.0, 32, [2 * (64 + 3), 128, 128], False, False)
        self.sa3 = PNSADenseNet(64, 2.0, 16, [2 * (128 + 3), 256, 256], False, False)
        self.sa4 = PNSADenseNet(16, 4.0, 16, [2 * (256 + 3), 512, 512], False, False)
