...
def knn_indices_func_gpu(rep_pts: cuda.FloatTensor,  # (N, pts, dim)
    ...
    for n, qry in enumerate(rep_pts):
        ...
        mqry = qry.expand(n, m, d).transpose(0, 1)  # (m, n, d)
        # dist2 = torch.sum((mqry - mref)**2, 2).squeeze()  # (m, n)
        dist2 = torch.sum((mqry - mref)**2, 2)  # (m, n)
        _, inds = torch.topk(dist2, k*D + 1, dim=1, largest=False)  # (m, k*d + 1)
        region_idx.append(inds[:, 1::D])  # (m, k)

    region_idx = torch.stack(region_idx, dim=0)  # (b, m, k)
    return region_idx
