...
def train():
    ...
    criterion = PointnetCriterion()  # no parameter
    ...
    for batch_idx_train in range(start_iter, batch_num):
        ...
        semantic3d_points_prob, _, _ = run_model(model, semantic3d_points_tensor.permute(0, 2, 1))  # (B, sample_num, num_classes), (B, sample_num, 3)
...
def val_one_epoch(model, dataset_relevant, val_writer, device, batch_idx_train):
    ...
    for batch_val_idx in tqdm(range(batch_num_val)):
        ...
        points_prob, _, _ = run_model(model, points_tensor.permute(0, 2, 1), validate=False)  # (B, sample_num, num_classes)
