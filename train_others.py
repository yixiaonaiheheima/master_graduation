...
parser.add_argument('--batch_size_train', type=int, default=12,
...
parser.add_argument('--sample_num', help='downsample number before feed to net', type=int, default=2048)
...
def train():
    ...
    for batch_idx_train in range(start_iter, batch_num):
        ...
        # normalize data
        semantic3d_points_sampled = basics_util.normalize_data(semantic3d_points_sampled)  # (B, sample_num, 3)
        # convert to tensor
        ...
        semantic3d_points_prob = run_model(model, semantic3d_points_tensor.permute(0, 2, 1))  # (B, sample_num, num_classes), (B, sample_num, 3)
        _, semantic3d_points_pred = torch.max(semantic3d_points_prob, dim=2)  # (B, sample_num)
        batch_loss = criterion(semantic3d_points_prob, semantic3d_labels_tensor)
        batch_loss.backward()
