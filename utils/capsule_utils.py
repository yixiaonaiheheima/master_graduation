...
import sys
from models.loss import distChamfer
...
class PrimaryPointCapsLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        u = torch.stack(u, dim=2)  # (B, 1024, prim_vec_size)
        return self.squash(u.squeeze())  # (B, 1024, prim_vec_size)
...
class LatentCapsLayer(nn.Module):
    ...
    def forward(self, x):
        """
        x: tensor(B, prim_caps_size, prim_vec_size)
        return: tensor(B, latent_caps_size)
        """
        u_hat = torch.squeeze(torch.matmul(self.W, x[:, None, :, :, None]), dim=-1)  # (B, latent_caps_size, prim_caps_size, latent_vec_size)
        ...
        for iteration in range(num_iterations):
            ...
            if iteration == num_iterations - 1:
                v_j = self.squash(torch.sum(c_ij[:, :, :, None] * u_hat, dim=-2, keepdim=True))  # (B, latent_caps_size, 1, latent_vec_size)
            else:
                v_j = self.squash(torch.sum(c_ij[:, :, :, None] * u_hat_detached, dim=-2, keepdim=True))
                b_ij = b_ij + torch.sum(v_j * u_hat_detached, dim=-1)  # (B, latent_caps_size, prim_caps_size)
        return v_j.squeeze(-2)  # (B, latent_caps_size, latent_vec_size)
...
class PointGenCon(nn.Module):
    ...
    def forward(self, x):
        """
        x: tensor(B, bottle_size, N)
        return: tensor(B, bottle_size/4, N)
        """
        x = F.relu(self.bn1(self.conv1(x)))
        x = F.relu(self.bn2(self.conv2(x)))
...
class CapsDecoder(nn.Module):
    ...
    def forward(self, x):
        """
        x: tensor(B, latent_caps_size, latent_vec_size)
        """
        outs = []
        for i in range(0, self.nb_primitives):
            rand_grid = Variable(torch.cuda.FloatTensor(x.size(0), 2, self.latent_caps_size))
            rand_grid.data.uniform_(0, 1)
            y = torch.cat((rand_grid, x.transpose(2, 1)), 1).contiguous()  # (B, lantent_vec_size + 2, latent_caps_size)
            outs.append(self.decoder[i](y))  # [(B, 3, latent_caps_size)]
        return torch.cat(outs, 2).contiguous()  # (B, 3, N)
...
class PointCapsNet(nn.Module):
    ...
    def forward(self, data):
        ...
        data: tensor(B, 3, N)
        """
        x1 = self.conv_layer(data)  # (B, 128, N)
