...
class DenseNet1D(nn.Module):
    def __init__(self, channel_list, seblock=False):
        ...
        self.conv4 = torch.nn.Conv1d(channel_list[0] + channel_list[1] + channel_list[1] + channel_list[1], channel_list[2], 1)
...
class DenseNet2D(nn.Module):
    def __init__(self, channel_list, seblock=False):
        """
        Input:
            channel_list: a list for input, middle and output data dimension, length is 3
            seblock: wheather use squeeze-and-excitation networks or not
        """
        ...
        self.conv4 = torch.nn.Conv2d(channel_list[0] + channel_list[1] + channel_list[1] + channel_list[1], channel_list[2], 1)
...
class PNSADenseNet_GAT(nn.Module):
    def __init__(self, npoint, radius, nsample, channel_list, droupout, alpha, normalize_radius=False, group_all=False):
        """
        Input:
            ...
            alpha=0.2
        """
        super(PNSADenseNet_GAT, self).__init__()
        self.npoint = npoint
        self.radius = radius
    ...
    def forward(self, xyz, points):
        ...
        if self.group_all:
            ...
            new_xyz, new_points, grouped_xyz, fps_points = \
                sample_and_group(self.npoint, self.radius, self.nsample, xyz, points, self.normalize_radius, returnfps=True)

        new_points = new_points.permute(0, 3, 2, 1)  # [B, C+D, nsample,npoint]
        fps_points = fps_points.unsqueeze(3).permute(0, 2, 3, 1)  # [B, C+D, 1,npoint]
        new_points_dense = self.pnsadensesnet(new_points)  # [B, D', nsample, npoint]
        fps_points_dense = self.pnsadensesnet(fps_points)  # [B, D', 1, npoint]
        # fps_points_dense = self.pnsadensesnetfps_points(fps_points)  # [B, D', 1, npoint]
        new_points_dense_GAT = self.GAT(center_xyz=new_xyz,  # (B, np, 3)
                                        center_feature=fps_points_dense.squeeze(2).permute(0, 2, 1),  # (B, np, D')
                                        grouped_xyz=grouped_xyz,  # (B, np, ns, 3)
                                        grouped_feature=new_points_dense.permute(0, 3, 2, 1))  # (B, np, ns, D')

        return new_xyz, new_points_dense_GAT


class PNSADenseNet_Dual(nn.Module):
    def __init__(self, npoint, radius, nsample, channel_list, normalize_radius=False, returnfps=False, group_all=False):
        """
        Input:
            npoint: keyponts number to sample
            radius: sphere radius in a group
            ...
            seblock: wheather use squeeze-and-excitation networks or not
        """
        super(PNSADenseNet_Dual, self).__init__()
        self.npoint = npoint
        self.radius = radius
        self.nsample = nsample
        self.returnfps = returnfps
        self.group_all = group_all
        self.normalize_radius = normalize_radius
        self.pnsadensesnet = DenseNet2D(channel_list, seblock=False)
        last_channel = channel_list[2]
        self.DualAttention = DualAttention(last_channel)
    ...
    def forward(self, xyz, points):
        ...
        Input:
            ...
            points: input points data, [B, N, D]
        Return:
            new_xyz: sub sampled points position data, [B, Ns, C]
            new_points_res: output points data, [B, Ns, D']
        """
        if self.group_all:
            new_xyz, new_points = sample_and_group_all(xyz, points)
        else:
            new_xyz, new_points, grouped_xyz, fps_points =\
                sample_and_group(self.npoint, self.radius, self.nsample, xyz, points, self.normalize_radius, self.returnfps)

        new_points = new_points.permute(0, 3, 2, 1)  # [B, C+D, nsample,npoint]
        fps_points = fps_points.unsqueeze(3).permute(0, 2, 3, 1)  # [B, C+D, 1,npoint]
        new_points_graph = new_points - fps_points # [B, C+D, nsample,npoint]
        new_points_input = torch.cat([new_points_graph, new_points], dim=1)
        ...
        new_points_dense_dual = DualAttention(new_points_dense)  # [B, npoint, D']
...
#         v = (torch.arange(0, grid_dims[1]) / grid_dims[1] - 0.5).expand(grid_dims[0], -1).t().reshape(-1)  # (grid_dims[1] * grid_dims[0])
