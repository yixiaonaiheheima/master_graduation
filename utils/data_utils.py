...
def load_seg(filelist, max_line=None):
    points = []
    labels = []
    point_nums = []
    labels_seg = []
...
def load_h5(h5_path):
    ...
    return points, labels, point_nums, labels_seg, indices_split_to_full


def load_seg_xzy(filelist):
    points = []
    labels = []
    point_nums = []
    labels_seg = []
    ...
    for line in open(filelist):
        data = h5py.File(os.path.join(folder, line.strip()), 'r')
        xzy = data['data'][...].astype(np.float32)
        xyz = deepcopy(xzy)
        xyz[:, :, 1] = xzy[:, :, 2]
        xyz[:, :, 2] = xzy[:, :, 1]
        points.append(xyz)
