...
def compute_geometry_feature(cloud, num_neighbors=30, view_point=None):
    ...
    :return: geometry_feature (N, 7), normal(N, 3)
    """
    print("compute geometry_feature by sorting")
    ...
    omni_variace = np.zeros((N,))
    linearity = np.zeros((N,))
    eigenvalue_entropy = np.zeros((N,))
    normals_vertical_component = np.zeros((N,))
    ...
    return normalized_geometry, normals