...
def mp_indices_conv(pts, K=16):
    ...
    return torch.LongTensor(indices).unsqueeze(0)  # (1, npts, K)

def mp_indices_deconv(pts, pts_next, K):
    tree = BallTree(pts, leaf_size=2)
    _, indices = tree.query(pts_next, k=K)
    return torch.LongTensor(indices).unsqueeze(0)  # (1, npts_next, K)
...
def mp_indices_conv_reduction(pts, K, npts):
    ...
    for ptid in range(npts):
        ...
        # perform the search
        dist, ids = tree.query([pt], k=K)  # (1, K), (1, K)
        ids = ids[0]  # (K,)

        used[ids] += 1
        used[index] += 1e7

        indices.append(ids.tolist())  # (K,) -> (npts, K)
        pts_n.append(pt)  # (3,) -> (npts, 3)

    pts_n = np.array(pts_n)  # (npts, 3)

    return torch.LongTensor(indices).unsqueeze(0), torch.from_numpy(pts_n).float().unsqueeze(0)  # (1, npts, K), (1, npts, 3)
...
class LayerBase(nn.Module):
    ...
    def indices_conv_reduction(self, input_pts, K, npts):
        ...
        indices = self.pool.starmap(mp_indices_conv_reduction, pts)  # [((1, npts, K), (1, npts, 3)) * B]
        indices, pts = zip(*indices)  # (1, npts, K) * B, (1, npts, 3) * B
        indices = torch.cat(indices, dim=0).long().cuda()  # (B, npts, K)
        pts = torch.cat(pts, dim=0).float().cuda()  # (B, npts, 3)
        return indices, pts
