...
def main():
    ...
    root = args.folder if args.folder else '/home/yss/sda1/yzl/Data/NPM3D/'
    ...
    label = np.zeros(batch_size, dtype=np.int32)  # not be used...
    ...
    folders = [os.path.join(root, folder) for folder in ['training_10_classes', 'val_10_classes', 'test_10_classes']]
    columns = ["x", "y", "z", "reflectance"]
    for folder in folders:
        datasets = [filename[:-4] for filename in os.listdir(folder) if filename.endswith('.ply')]
        for dataset_idx, dataset in enumerate(datasets):
            filename_ply = os.path.join(folder, dataset + '.ply')
            print('{}-Loading {}...'.format(datetime.now(), filename_ply))
            ply_data = PlyData.read(filename_ply)
            ...
            if ('class', '<i4') in ply_data.dtype.descr:
                ...
                labels = np.zeros((xyzi.shape[0]))

            xyz, i = np.split(xyzi, (3,), axis=-1)
            i = i / 255 - 0.5
            ...
            for offset_name, offset in offsets:
                ...
                print('{}-Computing block id of {} points...'.format(datetime.now(), xyzi.shape[0]))
                ...
                xyz_blocks = np.floor((xyz - xyz_min) / block_size).astype(np.int)  # (n, 3)

                print('{}-Collecting points belong to each block...'.format(datetime.now(), xyzi.shape[0]))
                # blocks: (blocks_num, 3)
                blocks, point_block_indices, block_point_counts = np.unique(xyz_blocks, return_inverse=True,
                                                                            return_counts=True, axis=0)
                block_point_indices = np.split(np.argsort(point_block_indices), np.cumsum(block_point_counts[:-1]))  # (blocks_num,)
                print('{}-{} is split into {} blocks.'.format(datetime.now(), dataset, blocks.shape[0]))

                block_to_block_idx_map = dict()
                for block_idx in range(blocks.shape[0]):
                    block = (blocks[block_idx][0], blocks[block_idx][1])  # assert(blocks[block_idx][2] == 0)
                ...
                for block_idx in range(idx_last_non_empty_block + 1):
                    point_indices = block_point_indices[block_idx]  # (n1,) where n1 > block_point_count_threshold
                    if point_indices.shape[0] == 0:
                        continue
                    block_points = xyz[point_indices]
                    block_min = np.amin(block_points, axis=0, keepdims=True)  # (n1, 3)
                    xyz_grids = np.floor((block_points - block_min) / args.grid_size).astype(np.int)  # (n1, 3)
                    ...
                    for grid_idx in range(grids.shape[0]):
                        ...
                        if repeat_num > 1:
                            ...
                            # grid count will be grid_point_count_avg
                ...
                for block_idx in range(idx_last_non_empty_block + 1):
                    ...
                    # point_num_avg >= max_point_num
                    point_num_avg = int(math.ceil(block_point_num * 1.0 / block_split_num))
                    point_nums = [point_num_avg] * block_split_num
                    # make sure sum(point_num) == block_point_num
                    point_nums[-1] = block_point_num - (point_num_avg * (block_split_num - 1))
                    starts = [0] + list(np.cumsum(point_nums))  # (block_split_num+1,)

                    np.random.shuffle(point_indices)
                    block_points = xyz[point_indices]
                    block_min = np.amin(block_points, axis=0, keepdims=True)  # (3,)
                    block_max = np.amax(block_points, axis=0, keepdims=True)  # (3,)
                    block_center = (block_min + block_max) / 2  # (3,)

                    block_center[0][-1] = block_min[0][-1]
                    block_points = block_points - block_center  # align to block bottom center, (n1, 3)
                    x, y, z = np.split(block_points, (1, 2), axis=-1)  # (n1, 1), (n1, 1), (n1, 1)
                    # use order 'xyzi'
                    block_xyzi = np.concatenate([x, y, z, i[point_indices]], axis=-1)  # (n1, 7)
                    block_labels = labels[point_indices]  # (n1,)
                    ...
                    for block_split_idx in range(block_split_num):
                        ...
                        data[idx_in_batch, 0:point_num, ...] = block_xyzi[start:end, :]
