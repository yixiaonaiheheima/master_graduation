...
def main():
    ...
    root = args.folder if args.folder else '/home/yss/sda1/yzl/Data/NPM3D_add/'

    # splits = ['training_10_classes', 'val_10_classes', 'test_10_classes']
    splits = ['training_10_classes', 'val_10_classes']
    split_filelists = dict()
    for split in splits:
        split_filelists[split] = ['./%s/%s\n' % (split, filename) for filename in os.listdir(os.path.join(root, split))
                                  if filename.endswith('.h5')]

    train_h5 = split_filelists['training_10_classes']
    random.shuffle(train_h5)
    train_list = os.path.join(root, 'train_data_files.txt')
    if not os.path.exists(os.path.join(root, 'filelists')):
    ...
    val_h5 = split_filelists['val_10_classes']
    ...
    if 'test_10_classes' in splits:
        test_h5 = split_filelists['test_10_classes']
