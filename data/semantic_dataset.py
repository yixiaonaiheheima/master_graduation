...
class SemanticFileData:
    def __init__(
        ...
        # self.geometry = np.stack([self.geometry[:, 0], self.geometry[:, 4]], axis=1)  # (N, 2)
    ...
    def sample_batch(self, batch_size, num_points_per_sample):
        ...
        return (
            np.array(batch_points_centered),  # (B, N, 3)
            np.array(batch_points_raw),  # (B, N, 3)
            np.array(batch_labels),  # (B, N)
            np.array(batch_colors),  # (B, N, 3)
            np.array(batch_geometry)  # (B, N, 1)
        )
